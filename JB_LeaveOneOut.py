...
def create_counting_features(thistrain, field='people_id'):
    ...
    for name in thistrain.columns.values:
        if name not in set(['people_id', field, 'activity_id', 'fold', 'char_38', 'outcome', 'date_x', 'date_y']) and \
    ...
    for name in train.columns.values:
        ...
        if name in set(['char_38']):
            ...
            if test is not None:
                new_feat=np.array([test[name]], dtype=np.float).T
                test_feats=np.append(test_feats, new_feat, 1)
        if False and name in set(['people_id', 'group_1']):
            print(name, end=', ')
            feature_names=feature_names + [name]
            new_feat=np.array([c[name][train[name]]]).T
            train_feats=np.append(train_feats, new_feat, 1)
...
print('Create 5-fold split for stacking ... ')
np.random.seed(0)
...
weight2v=np.array(train.groupby('group_1')['people_id'].unique().apply(lambda x: len(x)).apply(lambda x: 5*x*0.2**x)[train['group_1']])
weight2= np.array(1./train['group_1'].value_counts()[train['group_1']])
weight3=np.array(train.groupby('group_1')['people_id'].unique().apply(lambda x: len(x)).apply(lambda x: 5*x*0.2**x)[train['group_1']])
weight3=weight3 * np.array(train['people_id'].value_counts().apply(lambda x: 1/x)[train['people_id']])
...
gbm1 = xgb.train(params, dtrain, num_boost_round=111, evals=[(dtrain, 'train')], verbose_eval=15)
...
gbm2 = xgb.train(params2, dtrain2, num_boost_round=80, evals=[(dtrain2, 'train')], verbose_eval=15)
...
submission.to_csv('submit_xgb_new_valid.csv', index=False)
